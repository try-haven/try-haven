...
export default function SwipeableCard({
  ...
  useEffect(() => {
    ...
    if (recentChanges.length > 0) {
      const latestChange = recentChanges[recentChanges.length - 1]; // Most recent is last
      const daysAgo = Math.floor((Date.now() - new Date(latestChange.timestamp).getTime()) / (24 * 60 * 60 * 1000));
      setRecentPriceChange({
        type: latestChange.new_price < latestChange.old_price ? "decrease" : "increase",
        daysAgo: daysAgo,
        oldValue: latestChange.old_price,
